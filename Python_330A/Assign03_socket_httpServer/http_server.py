...
def response_path(path):
    ...
    if path == '/':
        return bytearray("\r\n".join(os.listdir('webroot/')),\
            'utf-8'), bytearray('text/plain', 'utf-8')

    if path == '/images':
        return bytearray("\r\n".join(os.listdir('webroot/images')),\
            'utf-8'), bytearray('text/plain', 'utf-8')

    lookup = open('webroot' + path, 'rb')
    content = lookup.read()
    mime, _ = mimetypes.guess_type('webroot'+ path)
    mime_type = bytearray(mime, 'utf-8')

    # use mime type to spares types and extract content, if none do the other 2?
