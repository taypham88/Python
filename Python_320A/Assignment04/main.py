...
def load_users(filename, database):
    ...
    try:
        ...
        logging.info(f"UsersTable not created, duplicate user {UsersTable.user_id} found")
        return False
    return True
...
def load_status_updates(filename, database):
    ...
    try:
        ...
        logging.info(f"StatusTable not created, duplicate status {StatusTable.status_id} found")
        return False
    return True
...
def search_all_status_updates(user_id, status_collection):
    '''Takes user_id and status collection
    to return a tuple list of all matching statues
    '''
    return user_status.UserStatusCollection.\
        search_all_status_updates(status_collection, user_id)


def filter_status_by_string(filter_string : str, status_collection):
    '''Takes user_id and status collection
    to return a tuple list of all matching statues
    '''
    return user_status.UserStatusCollection.\
        filter_status_by_string(status_collection, filter_string)

def yes_or_no(question : str):
    '''input check for a yes or no question.'''
