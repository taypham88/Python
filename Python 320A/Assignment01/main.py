...
#Instance of 'TextFileReader' has no 'loc' member (no-member) for E1101
...
def save_status_updates(filename, status_collection):
    ...
    - Otherwise, it returns True.
    '''
    # headers = ['STATUS_ID','USER_ID','STATUS_TEXT']
    # if len(filename) > 0:
    #     with open(filename, 'w', encoding='utf-8', newline = '') as csvfile:
    #         csv_writer = csv.writer(csvfile, delimiter = ',')
    #         csv_writer.writerow(headers)
    #         for status in status_collection.database.values():
    #             row = [status.status_id, status.user_id, status.status_text]
