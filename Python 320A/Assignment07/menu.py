...
def load_users(user_collection):
    ...
    logging.info(f"load user csv total time: {(time.time()-init_time)*1000:.4f} Milliseconds.")
...
def load_status_updates(user_collection, status_collection):
    ...
    logging.info(f"load status csv total time: {(time.time()-init_time)*1000:.4f} Milliseconds.")
